...
vst.opcodes = {
	...
	get_program=ffi.C.effGetProgram,		
	program_name=ffi.C.effSetProgramName,
	get_program_name=ffi.C.effGetProgramName,	
	get_param_label = ffi.C.effGetParamLabel,	
	get_param_display = ffi.C.effGetParamDisplay,	
	get_param_name = ffi.C.effGetParamName,		
	get_vu = ffi.C.effGetVu,	
	set_sample_rate = ffi.C.effSetSampleRate,	
	set_block_size = ffi.C.effSetBlockSize,	
	mains_changed = ffi.C.effMainsChanged,	
	edit_get_rect = ffi.C.effEditGetRect,		
    edit_open = ffi.C.effEditOpen,		
    edit_close = ffi.C.effEditClose,		
    edit_draw = ffi.C.effEditDraw,	
    edit_mouse = ffi.C.effEditMouse,	
    edit_key = ffi.C.effEditKey,	
    edit_idle = ffi.C.effEditIdle,		
    edit_top = ffi.C.effEditTop,	
    edit_sleep= ffi.C.effEditSleep,	
    identify = ffi.C.effIdentify,		
	get_chunk = ffi.C.effGetChunk,		
	set_chunk = ffi.C.effSetChunk,		 
	max_opcode = ffi.C.effNumOpcodes,		       
    can_be_automated = ffi.C.effCanBeAutomated,	
	string_to_paramter = ffi.C.effString2Parameter,
	get_num_program_categories = ffi.C.effGetNumProgramCategories,
	get_program_name_indexed = ffi.C.effGetProgramNameIndexed,	
	copy_program =  ffi.C.effCopyProgram,	
	connect_input = ffi.C.effConnectInput,	
	connect_output = ffi.C.effConnectOutput,		
	...
	set_block_size_and_samplerate =  ffi.C.effSetBlockSizeAndSampleRate,	
	set_bypass =  ffi.C.effSetBypass,
	get_effect_name =effGetEffectName,				
	get_error_text = ffi.C.effGetErrorText,
	get_vendor_string = ffi.C.effGetVendorString,
	get_product_string =  ffi.C.effGetProductString,
	get_vendor_version =  ffi.C.effGetVendorVersion,
    vendor_specific = ffi.C.effVendorSpecific,
	can_do = ffi.C.effCanDo,	
	get_tail_size =  ffi.C.effGetTailSize,
	idle = ffi.C.effIdle,
	get_icon =  ffi.C.effGetIcon,
	set_view_position =  ffi.C.effSetViewPosition,
	get_parameter_properties = ffi.C.effGetParameterProperties,
	keys_required =  ffi.C.effKeysRequired,	
	get_vst_version = ffi.C.effGetVstVersion,
	...
	begin_set_program =  ffi.C.effBeginSetProgram,
	end_set_program =  ffi.C.effEndSetProgram,	
	get_speaker_arrangement =  ffi.C.effGetSpeakerArrangement,
	shell_get_next_plugin =  ffi.C.effShellGetNextPlugin,
	start_process =  ffi.C.effStartProcess,
	stop_process = ffi.C.effStopProcess,
	set_total_sample_to_process = ffi.C.effSetTotalSampleToProcess,
	set_pan_law =  ffi.C.effSetPanLaw,		
	begin_load_bank =  ffi.C.effBeginLoadBank,
	begin_load_program =  ffi.C.effBeginLoadProgram,
	set_process_precision =  ffi.C.effSetProcessPrecision,
	get_num_midi_input_channels =  ffi.C.effGetNumMidiInputChannels,
